...
This pipeline asses transposon absence polymorphisms (TAPs) using pair-end short read illumina data. It first calculates GC corrected read depth coverage (RD) per bin  across whole genome and then compares a test sample against a user defined control sample to detected significant changes of RD for every TE of at least 300 bp. The test is by performing a non-parametric test between sample and control bins populations (with a minimun of 30 bins per population). If the tested TE has a significant change in RD and also the TE RD is 10 times less than the average sample RD it is considered a TE absence polymorphism. 
